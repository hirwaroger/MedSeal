...
#[ic_cdk::update]
pub fn submit_doctor_verification_request(request: SubmitVerificationRequest) -> Result<String> {
    ...
    if let Some(user) = storage::get_user_by_principal(&caller_principal) {
        match user.role {
            UserRole::Doctor => {
                ...
                Ok(format!("Doctor verification request submitted successfully with ID: {}", verification_id))
            },
            _ => Err("Only doctors can submit doctor verification requests".to_string()),
        }
    } else {
